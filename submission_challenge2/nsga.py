...
class NSGAII(EA):
    ...
    def tell(self, population: np.ndarray, fitness: np.ndarray, save_checkpoint=False) -> None:
        ...
        # Select best n_parents individuals from combined population
    ...
    def dominates(self, individual: np.ndarray, other_individual: np.ndarray) -> bool:
        ...
        at_least_as_good = all(a >= b for a, b in zip(individual, other_individual))
        strictly_better = any(a > b for a, b in zip(individual, other_individual))
        ...
        for m in range(n_objectives):
            ...
            for k in range(1, n_solutions - 1):
                distance[sorted_idx[k]] += (
                    fitness[front[sorted_idx[k + 1]], m] - fitness[front[sorted_idx[k - 1]], m]
                ) / obj_range
